# Myra Agents — Dev Workspace — Agent Instructions
...
Each member has its own `AGENTS.md` — read that when working **inside** a member.
This file governs only the bootstrap scripts.

### Org repos NOT in the bootstrap workspace

`bootstrap.sh` clones only the 5 members above. Other Myra-Agents repos you may
be asked to touch aren't checked out here — clone them ad hoc (they have no
`dir`). The ones that come up:

| repo                     | what                                              | vis    |
|--------------------------|---------------------------------------------------|--------|
| `Myra-Agents.github.io`  | **the landing page** — Next.js site at <https://myra-agents.github.io> | public |
| `Antenna`                | separate public repo — **not** the landing page; don't conflate the two | public |

When someone says "the landing page" / "the landing site", they mean
**`Myra-Agents.github.io`**, not `Antenna`.
...
- **`app/AGENTS.md` → Debugging** — Tauri log file
...
- **`server/AGENTS.md` → Debugging** — the data dir `~/.myra-agents/`:
...
- **`hub/AGENTS.md` → Debugging** — local `bun run dev` stdout vs deployed Worker
...
- **`dev.sh`** — thin wrappers over each member's own scripts. Targets:
  ...
  **`app`/`app-demo` are browser-testable:** they export `MYRA_DEV_PORT` +
  ...
  AGENTS.md "Test against a REAL sidecar".
